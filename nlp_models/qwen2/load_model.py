...
GEMMA_MODEL_SIZES_LAYERS ={
    '7b' : 28,
}

def get_qwen2(lm_name, lm_size, lm_cache_dir, device_map='auto', precision=None, return_tokenizer_only=False):
    lm_size = lm_size.lower()
    if lm_name == "qwen2":
        name = f"Qwen/Qwen2-7B-Instruct"
    else:
...
def custom_device_map(lm_size, num_gpus):
    ...
    model_size = GEMMA_MODEL_SIZES_LAYERS[model_size]
